...
export class FirebaseDB {
  constructor() {
    ...
    this.data = {
      ...
      group_admins: this._createProxy('group_admins')
    }
  ...
  async read() {
    ...
    try {
      ...
      // Read group admins
      const groupAdminsSnapshot = await this.db.collection(this.collections.group_admins).get()
      groupAdminsSnapshot.forEach(doc => {
        const data = doc.data()
        const id = data._id || doc.id
        this._data.group_admins[id] = restoreFromFirebase(data)
      })
  ...
  /**
   * Get group admins by JID
   */
  async getGroupAdmins(jid) {
    if (this._data.group_admins[jid]) return this._data.group_admins[jid]

    if (this.db) {
      try {
        const doc = await this.db.collection(this.collections.group_admins).doc(jid.replace(/[\/\.]/g, '_')).get()
        if (doc.exists) {
          this._data.group_admins[jid] = restoreFromFirebase(doc.data())
          return this._data.group_admins[jid]
        }
      } catch (error) {
        console.error('[Firebase] Get group admins error:', error.message)
      }
    }
  ...
  /**
   * Set group admins data
   */
  async setGroupAdmins(jid, data) {
    this._data.group_admins[jid] = { ...this._data.group_admins[jid], ...data }
    this.dirty.group_admins.add(jid)

    if (this.db) {
      try {
        await this.db.collection(this.collections.group_admins).doc(jid.replace(/[\/\.]/g, '_')).set(
          { ...prepareForFirebase(this._data.group_admins[jid]), _id: jid },
          { merge: true }
        )
        this.dirty.group_admins.delete(jid)
      } catch (error) {
        console.error('[Firebase] Set group admins error:', error.message)
      }
    }
